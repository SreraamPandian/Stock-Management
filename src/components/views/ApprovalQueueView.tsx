...
interface Props {
    viewAsRole?: 'SMD' | 'Finance';
    isReadOnly?: boolean;
}

export const ApprovalQueueView = ({ viewAsRole, isReadOnly = false }: Props = {}) => {
    ...
    const pendingStatus = effectiveRole === 'SMD' ? 'Pending - SMD' : 'Pending - Finance';
    const nextStatus = effectiveRole === 'SMD' ? 'Pending - Finance' : 'PO Issued';
    ...
    const handleAction = async () => {
        ...
        if (action === 'approve') {
            updatePRStatus(selectedPR.id, nextStatus, `${effectiveRole} approved. ${remarks}`, {
                smdApproved: effectiveRole === 'SMD' ? true : selectedPR.smdApproved,
            });
            setDone(`✓ ${selectedPR.id} approved and forwarded to ${effectiveRole === 'SMD' ? 'Finance' : 'PO Issuance'}`);
        } else {
            updatePRStatus(selectedPR.id, 'Rejected - Action Required', `Rejected by ${effectiveRole}. Reason: ${remarks}`);
    ...
    return (
        <div className={`p-8 max-w-7xl mx-auto space-y-8 ${isReadOnly ? 'opacity-95' : ''}`}>
            {/* Header */}
            <div>
                <h1 className="text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                    {effectiveRole === 'SMD'
                        ? <><CheckCircle className="w-8 h-8 text-purple-600" /> SMD Approval Queue {isReadOnly && <span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded ml-2">Read-Only View</span>}</>
                        : <><FileText className="w-8 h-8 text-blue-600" /> Finance Clearance Queue {isReadOnly && <span className="text-xs bg-slate-100 text-slate-500 px-2 py-1 rounded ml-2">Read-Only View</span>}</>
                    }
                </h1>
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden">
                <div className="p-5 border-b border-slate-50">
                    <h2 className="text-xs font-black text-slate-500 uppercase tracking-widest">
                        {queue.length} PR{queue.length !== 1 ? 's' : ''} Pending {effectiveRole === 'SMD' ? 'Executive Approval' : 'Finance Clearance'}
                    </h2>
                </div>
