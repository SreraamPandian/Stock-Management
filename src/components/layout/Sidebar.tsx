...
export const Sidebar = () => {
  ...
  const coreNavItems: { icon: any; label: Tab }[] = [
  ...
  return (
    <aside className="w-64 bg-white border-r border-slate-200 flex flex-col h-screen fixed left-0 top-0 z-20">
      ...
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
        ...
        {/* Super Admin & Procurement Officer — Finance Queue */}
        {(isSuperAdmin || role === 'Procurement Officer') && (
          <button
            onClick={() => setActiveTab('Finance Queue' as Tab)}
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${activeTab === 'Finance Queue'
              ? 'bg-blue-50 text-blue-700'
              : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
              }`}
          >
            <FileText className={`w-5 h-5 ${activeTab === 'Finance Queue' ? 'text-blue-600' : 'text-slate-400'}`} />
            Finance Queue
          </button>
        )}
        ...
        {isSuperAdmin && (
          <button
            ...
            className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${activeTab === 'Admin'
              ? 'bg-blue-50 text-blue-700'
              : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'
              }`}
          >
            <Settings className={`w-5 h-5 ${activeTab === 'Admin' ? 'text-blue-600' : 'text-slate-400'}`} />
            Admin
          </button>
        )}
